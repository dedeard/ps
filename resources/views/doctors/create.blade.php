    ...
    <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
      <div class="p-6 text-gray-900">
        <form method="POST" action="{{ route('doctors.store') }}">
          @csrf
          <x-inputs.text name="name" :placeholder="__('Full Name')" :label="__('Full Name')" label-required />
          <x-inputs.select name="specialization" :placeholder="__('Specialization')" :label="__('Specialization')" label-required>
            @foreach ($specializations as $specialist)
              <option value="{{ $specialist }}" @if (old('specialization') === $specialist) selected @endif>
                {{ $specialist }}
              </option>
            @endforeach
          </x-inputs.select>
